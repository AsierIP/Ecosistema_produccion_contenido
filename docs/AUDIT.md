...
¿Sabías que? no apareció en la búsqueda específica de Markdown, JSON y YAML en `content`, `.agents` y `docs` de Religion. Su alta se trata como un perfil propio del ecosistema: voz Kore bloqueada y logo neón sobre negro v04 aprobado según la configuración recuperada para ese canal; estilo visual provisional. Su ficha inicial está en `channels/sabias-que.json` y registra como fuente *Enciclopedia de las curiosidades: El libro de los hechos insólitos*, de Gregorio Doval. Las identidades Vibes, YouTube y TikTok permanecen sin resolver. No se infieren del nombre ni de la cuenta de Religion.
